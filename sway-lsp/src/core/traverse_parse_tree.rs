...
fn handle_function_declation(func: &FunctionDeclaration, tokens: &mut TokenMap) {
    tokens.insert(
        ...
        TokenType::Token(AstToken::FunctionDeclaration(func.clone())),
    );
    ...
    for parameter in &func.parameters {
        tokens.insert(
            ...
            TokenType::Token(AstToken::FunctionParameter(parameter.clone())),
        );
    }

    // TODO: implement the handle_custom_type fn
    ...
    match declaration {
        Declaration::VariableDeclaration(variable) => {
            ...
            if !variable.name.as_str().contains(TUPLE_NAME_PREFIX) {
                tokens.insert(
                    ...
                    TokenType::Token(AstToken::Declaration(declaration.clone())),
                );
            }
        ...
        Declaration::TraitDeclaration(trait_decl) => {
            ...
            for train_fn in &trait_decl.interface_surface {
                tokens.insert(
                    ...
                    TokenType::Token(AstToken::TraitFn(train_fn.clone())),
                );
            }
        ...
        Declaration::StructDeclaration(struct_dec) => {
            ...
            for field in &struct_dec.fields {
                tokens.insert(
                    ...
                    TokenType::Token(AstToken::StructField(field.clone())),
                );
            }
        }
        Declaration::EnumDeclaration(enum_decl) => {
            ...
            for variant in &enum_decl.variants {
                tokens.insert(
                    ...
                    TokenType::Token(AstToken::EnumVariant(variant.clone())),
                );
            }
        }
        Declaration::Reassignment(reassignment) => {
            ...
            tokens.insert(
                ...
                TokenType::Token(AstToken::Reassignment(reassignment.clone())),
            );
        }
        ...
        Declaration::AbiDeclaration(abi_decl) => {
            ...
            for trait_fn in &abi_decl.interface_surface {
                tokens.insert(
                    ...
                    TokenType::Token(AstToken::TraitFn(trait_fn.clone())),
                );

                for param in &trait_fn.parameters {
                    tokens.insert(
                        to_ident_key(&param.name),
                        TokenType::Token(AstToken::FunctionParameter(param.clone())),
                    );
                }
        ...
        Declaration::StorageDeclaration(storage_decl) => {
            for field in &storage_decl.fields {
                tokens.insert(
                    to_ident_key(&field.name),
                    TokenType::Token(AstToken::StorageField(field.clone())),
                );
            }
        }
    }
    ...
    match expression {
        ...
        Expression::StructExpression {
            ...
            for field in fields {
                tokens.insert(
                    to_ident_key(&field.name),
                    TokenType::Token(AstToken::Expression(field.value.clone())),
                );
